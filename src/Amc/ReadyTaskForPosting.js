...
export function ReadyTaskForPosting() {
        ...
        {showApprovedTaskGrid && (
          <div className="approved-task-row-container">
            {approvedCompanies.map((company) =>
              company.tasks
                .filter((task) => isTodayOrPast(task.targetPostingDate))
                .map((task) => (
                  <div
                  ...
                  >
                    <div className="cmpy-id-status">
                      ...
                      >
                        {isToday(task.targetPostingDate) ? (
                          <span
                            style={{
                              background: "#b82de6",
                              color: "#fff",
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      const token = localStorage.getItem("token");
                      await axios.put(
                        `${process.env.REACT_APP_API_URL}/task/update-task/${selectedApprovedTask._id}`,
                        { currentStatus: "Posted" },
                        { headers: { Authorization: `Bearer ${token}` } }
                      );
                      toast.success("Task status set to Posted!");
                      setSelectedApprovedTask({
                        ...selectedApprovedTask,
                        currentStatus: "Posted",
                      });
                      // Optionally refresh the grid/list if needed
                      fetchCompanyAllDetails();
                    } catch (err) {
                      console.error(
                        "Launch error:",
                  ...
                  disabled={selectedApprovedTask.currentStatus === "Posted"}
                >
                  {selectedApprovedTask.currentStatus === "Posted"
                    ? "Launched"
                    : "Launch"}
                </button>
